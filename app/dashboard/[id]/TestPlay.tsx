  ...
  const handleSelectedAnswer = (
    ...
    setSelectedAnswers((prev) => {
      ...
      if(existingAnswerIndex !== -1){
        const updated = [...prev];
          updated[existingAnswerIndex] = {questionId, optionId, text, point: isCorrect ? 1 : 0};
        return updated;
      }else{
        return [...prev, {questionId, optionId, text, point: isCorrect ? 1 : 0}]
      }
    })
  };
            ...
            <Stack direction={"column"}>
              ...
              {question.options.map((option: Option) => (
                <Box key={option.id}>
                  ...
                  <Button
                    onClick={() => {
                      handleSelectedAnswer(question.id, option.id, option.text);
                    }}
                  >
                    Select this
                  </Button>
